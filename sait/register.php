<?php
    ...
    if(isset($_POST["btn_submit_register"]) && !empty($_POST["btn_submit_register"])){
        ...
        if(isset($_POST["last_name"])){
            ...
            $_SESSION["error_mesaages"] .= "<p class='mesage_error'>Отсутствует поле с фамилией</p>";
            header("HTTP/1.1 301 Moved Permamemtly");
            header("Location: " . $address_site . "/form_register.php");
            exit();
        }
        ...
        if(isset($_POST["email"])){
            ...
            if(!empty($email)){
                ...
                //место кода для проверки формата почтового адреса и уго уникальность
        ...
        //место для кода добавления пользователя БД
        ...
        if(!$result_query_insert){
            ...
            $_SESSION["seccess_messages"] .="<p class='success_message'>Регистрация прошла успешно!! <br> Теперь Вы можете авторизоваться используя Ваш логин и пароль.</p>";
            header("HTTP/1.1 301 Moved Permanently");
            header("Location: " . $address_site . "/form_register.php");
        }
        ...
        exit("<p><strong>Ошибка!</strong>
        Отсутствует проверочный код, то есть код капчи. Вы можете перейти на <a href=".$address_site.">главную страницу</a>.</p>");
    }   

    }else{
        exit("<p><strong>Ошибка!</strong>
        Вы зашли на эту страницу напрямую, поэтому нет данных для обработки.
